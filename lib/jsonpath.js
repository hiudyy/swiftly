    ...
    while (i < n) {
        ...
        if (c === '[') {
            ...
            if (inner === '*') {
                ...
                // Full integer (optionally negative) -> index. A partial
                // number like "0:2" or "1,2" must NOT silently become index
                // 0/1 — it falls through to a key lookup (undefined) so
                // unsupported slice/filter/unions fail loudly, not wrongly.
...
export function queryJSON(data, path, fallback) {
    let p = String(path);
    // Tolerate the standard JSONPath root prefix: '$.store' == 'store'.
    if (p.startsWith('$')) {
        p = p.slice(1).replace(/^\./, '');
    }
