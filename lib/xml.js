...
function decodeXML(text) {
    ...
    return text.replace(/&(#x?[0-9a-fA-F]+|[a-zA-Z0-9]+);/g, (m, e) => {
        if (e[0] === '#') {
            ...
            return isNaN(code) ? m : decodeNumericEntity(code);
        }
...
function nodeToObject(node) {
    ...
    while (stack.length) {
        ...
        if (!ready) {
            // Re-visit later, but below the unprocessed children so they are
            // converted first (LIFO). Each node is re-pushed at most once per
            // unresolved child generation, so this stays linear.
            stack.push(n);
            for (const c of n.children) {
