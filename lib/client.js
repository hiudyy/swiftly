...
class CircuitBreaker {
    constructor(config = {}) {
        ...
        // Only one probe request is allowed while HALF-OPEN; the rest fail fast.
    ...
    async execute(command, domain) {
        ...
        // HALF-OPEN: let exactly one trial request through and reject any
        // concurrent requests until the trial verdict is known — otherwise a
        // burst of requests could all hit a still-recovering service.
        ...
        try {
            ...
            if (this.state === 'HALF-OPEN') {
                // The single trial failed: the service is still unhealthy, so
                // reopen immediately instead of accumulating threshold counts.
                this.state = 'OPEN';
                this.halfOpenTrialInFlight = false;
...
class HTTPClient {
    ...
    _validateRequestParams(method, url, data = null, config = {}) {
        ...
        // The URL must be truly absolute (contain '://') — otherwise a relative
        // string like "hello" would skip validation and explode later with a raw
        // TypeError instead of a typed ValidationError.
        if (
    ...
    _formatUrl(url, params) {
        ...
        try {
            // Apply baseURL if configured and url is relative. Check for a
            // scheme (`://`) rather than an 'http' prefix so relative paths
            // like "httpProxy/data" are not mistaken for absolute URLs.
            let fullUrl = url;
    ...
    _authVary(config) {
        // Header lookups are case-insensitive: users may set a lowercase
        // 'authorization'/'cookie' header, and node lowercases header names on
        // the wire. Otherwise an authenticated request could share a cache key
        // with an anonymous one (a credentials leak).
    ...
    _mergeConfig(customConfig) {
        ...
        // by object reference, but only for FLAT configs (no nested plain
        // objects). A reused config whose fields change between calls must be
        // honored: the snapshot check below catches primitive/object
        // replacement, and configs with nested objects are always re-merged so
        // deep mutations (e.g. cfg.params.page = 2) can never go stale.
        let hasNested = false;
    ...
    async request(method, url, data = null, customConfig = {}) {
        ...
        // Request deduplication for GET requests (skipped for streams). The key
        // includes the auth identity (same _authVary() the cache key uses) so
        // concurrent requests with different credentials — Authorization,
        // tokens or per-request Cookie headers — never share a response.
        const dedupKey = isGet && !config.stream
            ? `${upperMethod}:${formattedUrl}:${this._authVary(config)}`
            : null;
    ...
    async _executeRequest(method, url, data, config, startTime, routeKey, urlObj) {
        ...
        // `data` is mutated below (formData/compression replace it with a
        // prepared Buffer). Keep the original so a 307/308 redirect can re-run
        // the preparation in the fresh request instead of re-sending the raw
        // prepared bytes without their Content-Type/Content-Encoding headers.
        ...
        // matched case-insensitively so a user's lowercase header wins)
        const hasAuthHeader = Object.keys(options.headers)
        ...
        // Response recovered by an error interceptor (rejected handler returned
        // a value): fed back into the success path instead of retrying/raising.
        ...
        if (config.circuitBreaker && config.circuitBreaker.enabled) {
            ...
            if (!circuitBreaker) {
                ...
                // Forward the breaker's internal transitions to the client's
                // event emitter so `client.on('circuit:open', ...)` (and the
                // close/half-open/rejected events) fire as documented.
        ...
        while (attempt < config.retries) {
            try {
                // A response recovered by an error interceptor skips the wire
                // and flows through the normal success path.
                let response;
                ...
                if (config.followRedirects && [301, 302, 303, 307, 308].includes(response.status)) {
                    ...
                    // Browsers/axios semantics: 301/302/303 convert POST (and
                    // other non-GET/HEAD methods) to GET and drop the body,
                    // otherwise a form POST would be re-submitted to the
                    // redirect target. 307/308 always preserve method + body.
                    ...
                    // Converter URL relativa em absoluta se necessário. Check
                    // for a scheme (not an 'http' prefix) so a Location like
                    // "httpProxy/data" resolves against the current URL.
                ...
                // Response error interceptors (the rejected handlers of
                // `interceptors.response.use`) run here: they can inspect the
                // failure (e.g. refresh a token on 401) and, by returning a
                // value, recover the request with that response.
    ...
    _sendPayload(req, data, config = {}) {
        ...
        // Enforce the max body length guard (opt-in, default Infinity).
    ...
    _handleResponseStream(res, options, resolve, reject) {
        ...
        stream.on('data', chunk => {
            ...
            // Enforce the max content length guard (opt-in, default Infinity).
    ...
    _makeRequest(protocol, options, data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Informational event for pooled-socket reuse observability.
            req.once('socket', () => {
            ...
            // `config.timeout` is a per-request socket timeout: destroy the
            // request when the socket fires 'timeout' — Node only emits the
            // event, so without a listener the request would hang forever.
    ...
    _processResponse(response, responseType) {
        ...
        try {
            ...
            if (result && typeof result.then === 'function') {
                return result.then(
                    finish,
                    (error) => { throw this._wrapTransformError(error, response, type); }
                );
            }
    ...
    async _compressData(data, config = this.config) {
        ...
        // Use the per-request merged config (not the client global) so a
        // request-level `compression: { request: false }` is honored.
    ...
    async _createFormData(data) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            if (Buffer.isBuffer(value) || (value && (
                ...
                ArrayBuffer.isView(value.buffer)
            ))) {
                // Handle file uploads. Accepts a Buffer / TypedArray passed
                // directly, or a file descriptor { name, type, buffer } where
                // `buffer` is a Buffer, TypedArray or raw (Shared)ArrayBuffer.
                // byteOffset/byteLength are honored so pooled buffers are not
                // serialized whole.
    ...
    async subscribe(url, callbacks = {}, config = {}) {
        ...
        // SSE connections need the same auth + cookie semantics as regular
        // requests: apply the auth helpers (unless an explicit Authorization
        // header is set) and send cookies from the jar.
        if (!options.headers['Authorization']) {
            if (options.auth && options.auth.username !== undefined) {
                options.headers['Authorization'] = 'Basic ' +
